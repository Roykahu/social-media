...
import { Plus, Pencil, Trash2, Settings2, Sparkles, Search, Users, Film, Megaphone, ChevronDown, ChevronRight } from "lucide-react";
...
export default function ConfigsPage() {
  ...
  const openEdit = (config: Config) => {
    ...
    setForm({
      ...
      currentOfferName: config.currentOfferName || "",
      bookingLink: config.bookingLink || "",
      urgencyLine: config.urgencyLine || "",
    });
    setDialogOpen(true);
  };
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto glass-strong rounded-2xl border-white/[0.08]">
            ...
            <div className="space-y-5 pt-2">
              ...
              <div className="rounded-xl glass border-white/[0.08]">
                <button
                  type="button"
                  onClick={() => setCtaOpen(!ctaOpen)}
                  className="flex w-full items-center justify-between px-4 py-3 text-sm font-medium text-left hover:bg-white/[0.03] rounded-xl transition-colors"
                >
                  <span className="flex items-center gap-1.5">
                    <Megaphone className="h-3.5 w-3.5 text-orange-400" />
                    Training CTA Settings
                  </span>
                  {ctaOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
                </button>
                {ctaOpen && (
                  <div className="space-y-4 px-4 pb-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g. Book a Claude training for your team"
                        className="mt-1.5 rounded-xl glass border-white/[0.08] h-11"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g. https://roykahuthu.com"
                        className="mt-1.5 rounded-xl glass border-white/[0.08] h-11"
                      />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Current Offer Name</Label>
                      <Input
                        value={form.currentOfferName}
                        onChange={(e) => setForm({ ...form, currentOfferName: e.target.value })}
                        placeholder="e.g. Claude Cowork Masterclass — April 2026"
                        className="mt-1.5 rounded-xl glass border-white/[0.08] h-11"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g. https://roykahuthu.com/book"
                        className="mt-1.5 rounded-xl glass border-white/[0.08] h-11"
                      />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Urgency Line</Label>
                      <Input
                        value={form.urgencyLine}
                        onChange={(e) => setForm({ ...form, urgencyLine: e.target.value })}
                        placeholder="e.g. Only 3 corporate spots left this month"
                        className="mt-1.5 rounded-xl glass border-white/[0.08] h-11"
                      />
                    </div>
                  </div>
