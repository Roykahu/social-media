...
import type { Config, Creator, Video, Carousel } from "./types";
...
// Configs
const CONFIG_COLUMNS = ["id", "configName", "creatorsCategory", "analysisInstruction", "newConceptsInstruction", "ctaText", "websiteUrl", "currentOfferName", "bookingLink", "urgencyLine"];
...
// Carousels
const CAROUSEL_COLUMNS = ["id", "topic", "pillar", "sourceVideoId", "slides", "thumbnailPrompts", "dateCreated"];

export function readCarousels(): Carousel[] {
  return readCsv<Carousel>("carousels.csv");
}

export function writeCarousels(carousels: Carousel[]) {
  writeCsv("carousels.csv", carousels as unknown as Record<string, unknown>[], CAROUSEL_COLUMNS);
}

export function appendCarousel(carousel: Carousel) {
